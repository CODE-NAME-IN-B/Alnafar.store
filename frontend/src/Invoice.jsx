...
export default function Invoice({ cart, total, totalSize = 0, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
      <div className="bg-gray-900 w-full sm:max-w-lg rounded-t-2xl sm:rounded-xl shadow-2xl border border-gray-700 max-h-[92vh] sm:max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="p-4 sm:p-6 border-b border-gray-700 sticky top-0 bg-gray-900 z-10">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="text-gray-400 hover:text-white text-2xl p-2 min-w-[44px] min-h-[44px] flex items-center justify-center"
              aria-label="إغلاق"
            >
              ×
            </button>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 pb-4 sm:pb-0">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-3.5 min-h-[48px] border border-gray-600 text-gray-300 rounded-lg hover:bg-gray-800 transition-colors font-medium"
            >
              إلغاء
            </button>
            <button
              ...
              className="flex-1 px-4 py-3.5 min-h-[48px] bg-gradient-to-r from-primary to-emerald-500 hover:from-primary-dark hover:to-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-lg transition-all duration-300 flex items-center justify-center gap-2"
            >
              {isProcessing ? (
                <>

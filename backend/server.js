...
async function initializeDatabase() {
  // --- ترحيل جداول الباقات أولاً لضمان تجنب أخطاء 500 ---
  try {
    ...
    await exec(`CREATE TABLE IF NOT EXISTS package_games (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        package_id INTEGER NOT NULL,
        game_id INTEGER NOT NULL
      );`);
    // تحقق من وجود عمود id للكشف عن الهياكل القديمة
    try { 
      const test = await get('SELECT id FROM package_games LIMIT 1'); 
    } catch (e) {
      console.log('[Migration] Broken package_games detected, recreating...');
      await exec('DROP TABLE IF EXISTS package_games; DROP TABLE IF EXISTS packages;');
      await exec(`CREATE TABLE packages (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, price REAL NOT NULL, category_id INTEGER, is_active INTEGER DEFAULT 1, created_at DATETIME DEFAULT CURRENT_TIMESTAMP);`);
      await exec(`CREATE TABLE package_games (id INTEGER PRIMARY KEY AUTOINCREMENT, package_id INTEGER NOT NULL, game_id INTEGER NOT NULL);`);
    }
  } catch (e) {
    console.error('[DB] Critical error in packages migration:', e.message);
  }
...
// Get all packages (admin) - Optimized with single query
app.get('/api/packages', authMiddleware, async (req, res) => {
  try {
    const rows = await all(`
      SELECT p.*, g.id as g_id, g.title as g_title, g.image as g_image, g.price as g_price
      FROM packages p
      LEFT JOIN package_games pg ON p.id = pg.package_id
      LEFT JOIN games g ON pg.game_id = g.id
      ORDER BY p.created_at DESC
    `);
    
    // Group rows by package id
    const packagesMap = new Map();
    for (const row of rows) {
      if (!packagesMap.has(row.id)) {
        const { g_id, g_title, g_image, g_price, ...pkgData } = row;
        packagesMap.set(row.id, { ...pkgData, packageGames: [] });
      }
      if (row.g_id) {
        packagesMap.get(row.id).packageGames.push({
          id: row.g_id,
          title: row.g_title,
          image: row.g_image,
          price: row.g_price
    ...
    res.json({ success: true, packages: Array.from(packagesMap.values()) });
  } catch (error) {
    console.error('[API] Fetch packages error:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch packages', error: error.message });
  }
});

// Get active packages for a category (frontend) - Optimized with single query
app.get('/api/packages/active/:categoryId', async (req, res) => {
  try {
    ...
    console.error('[API] Fetch active packages error:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch active packages', error: error.message });
  }
});

// Debug route - ADMIN ONLY
app.get('/api/debug/db-schema', authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    res.json({ success: true, dbType: dbAdapter.getDbType(), tables: schema });
  } catch (error) {
...
app.put('/api/packages/:id', authMiddleware, async (req, res) => {
  try {
    const { name, price, category_id, game_ids, is_active } = req.body;
    await run(
      'UPDATE packages SET name = ?, price = ?, category_id = ?, is_active = ? WHERE id = ?',
    ...
    if (game_ids && game_ids.length > 0) {
      for (const gameId of game_ids) {
        await run('INSERT INTO package_games (package_id, game_id) VALUES (?, ?)', [req.params.id, gameId]);
      }
    }
...
app.put('/api/packages/:id/toggle', authMiddleware, async (req, res) => {
  try {
    const pkg = await get('SELECT is_active FROM packages WHERE id = ?', [req.params.id]);
    if (!pkg) return res.status(404).json({ success: false, message: 'Package not found' });
    
    const newStatus = pkg.is_active ? 0 : 1;
    await run('UPDATE packages SET is_active = ? WHERE id = ?', [newStatus, req.params.id]);
    res.json({ success: true, is_active: newStatus });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Failed to toggle package', error: error.message });
  }
});
...
app.get('/api/orders/:id', async (req, res) => {
  try {
    ...
    const order = await get('SELECT invoice_number, status, items, total, total_size_gb, estimated_minutes, discount, final_total, paid_amount, customer_name, customer_phone, created_at FROM invoices WHERE invoice_number = ? OR id = ?', [id, id]);
...
app.post('/api/seed-ps4-force', authMiddleware, async (req, res) => {
  const folder = 'Ps4';
...
app.post('/api/invoices', generalLimiter, async (req, res) => {
  try {
    ...
    const status = paidAmount <= 0 ? 'pending' : (paidAmount >= computedFinal ? 'completed' : 'pending');
...
app.put('/api/invoices/:id/payment', authMiddleware, async (req, res) => {
  try {
    ...
    const updatedInvoiceForStatus = await get('SELECT * FROM invoices WHERE id = ?', [id]);
    const finalTotal = (updatedInvoiceForStatus.total || 0) - (updatedInvoiceForStatus.discount || 0);
    const newPaidAmount = updatedInvoiceForStatus.paid_amount || 0;
    const newStatus = newPaidAmount <= 0 ? 'pending' : (newPaidAmount >= finalTotal ? 'completed' : 'pending');
